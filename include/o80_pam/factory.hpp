...
StandalonePtr get_standalone(
    std::string type,
    int id,
    const pam_interface::Configuration<NB_DOFS> &config)
{
    std::string segment_id =
        std::string(SEGMENT_ID_SUFFIX) + type + std::to_string(id);
    std::string object_id = std::string(OBJECT_ID) + std::to_string(id);

    std::shared_ptr<Driver> driver =
        std::make_shared<Driver>(config) return std::make_shared<
            Standalone<QUEUE_SIZE, NB_ACTUATORS>>(
            driver, config.server_frequency, segment_id, object_id);
}
                    ...
                    std::atomic<bool> &external_running)
{
    StandalonePtr standalone =
        internal::get_standalone<pam_interface::RealDriver>(type, id, config);
    standalone->start();
...
StandalonePtr get_dummy(int id,
                        const pam_interface::Configuration<NB_DOFS> &config)
{
    return internal::get_standalone<pam_interface::DummyDriver>(
        "_dummy_", id, config);
}
...
StandalonePtr get_real(int id,
                       const pam_interface::Configuration<NB_DOFS> &config)
{
    return internal::get_standalone<pam_interface::RealDriver>(
        "_real_", id, config);
}
...
{
    internal::run_standalone<QUEUE_SIZE, NB_DOFS, pam_interface::DummyDriver>(
        "_dummy_", id, config, bursting, running);
}
...
void run_real_standalone(int id,
                         ...
                         std::atomic<bool> &running)
{
    internal::run_standalone<QUEUE_SIZE, NB_DOFS, pam_interface::RealDriver>(
        "_real_", id, config, bursting, running);
}
};
