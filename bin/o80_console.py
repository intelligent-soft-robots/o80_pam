...
def _get_status(frontend, config):
    ...
    for dof, found in enumerate(observation.ref_founds):
        if not found:
            positions[dof] = None
    s = [
        "iteration: {} | frequency {:.2f} | time stamp {}\n".format(
            observation.iteration, observation.frequency, observation.time_stamp
        )
    ]
    for dof in range(config.nb_dofs):
        d = [dof]
        for ago in [0, 1]:
            d.append([observation.observed[dof][ago], observation.desired[dof][ago]])
...
class _Console:
    ...
    @classmethod
    def init(cls, permanent_info, config, frontend):
...
def _console(config, frontend):
    ...
    _Console.init(_permanent_info(config), config, frontend)
...
def _configure():
    ...
    config.add_option(
        "width", 100, "number of characters used for pressure bars", int, [Positive()]
    )
    change_all = False
