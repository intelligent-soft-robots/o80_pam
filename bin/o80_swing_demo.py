...
def run(config, frontend):
    ...
    nb_dofs = 4
    p0 = {"ago": (8000, 8000, 24000, 24000), "antago": (24000, 24000, 8000, 8000)}
    p1 = {"ago": (24000, 24000, 8000, 8000), "antago": (8000, 8000, 24000, 24000)}

    for iteration in range(config.nb_iterations):
        logging.info("iteration {} / {}".format(iteration, config.nb_iterations))
...
def _configure():
    ...
    config.add_option(
        "relax_pressure", 8000, "starting and ending pressure", int, [Range(0, 22000)]
    )
    config.add_option(
        "nb_iterations", 10, "number of swing motions to perform", int, [Positive()]
    )
    change_all = False
